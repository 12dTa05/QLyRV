...
namespace QLyRV
{
    public partial class TaiKhoan : Form
    {
        ...
        {
            //string chucvuQuery = "SELECT MaDV FROM DONVI where TonTai = 1 and MaDV != 'admin'";
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //string query = " SELECT  tk.TenDN, dv.TenDV,tk.MaNhom, tk.Khoa from TAIKHOAN tk, DONVI dv where tk.TDN = @MaDV and tk.Khoa = @tt";
        ...
        {
            string add = " insert into DONVI values @TenDN, @tendv, @ct, 0" + "\n insert into TAIKHOAN values @TenDN, @MK, @MaNhom, 1";
            string connectionString = conn_string;
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                ...
                {
                    cmd.Parameters.AddWithValue("@tendv", textBox5.Text.ToString());
        ...
        {
            string add = "update TAIKHOAN set Khoa = 1 where TDN = @TDN" + "\n update DONVI set TonTai = 0 where MaDV = @TDN";
            string connectionString = conn_string;
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
